...
namespace MBMS_RT {
  class ContentStream {
    public:
      ContentStream(std::string base, std::string flute_if, CacheManagement& cache, DeliveryProtocol protocol);//, const libconfig::Config& cfg);
      ...
      { if (_flute_receiver) _flute_receiver->handle_received_data(buffer, size);}
