...
MBMS_RT::RestHandler::RestHandler( const std::string& url, const CacheManagement& cache,
    ...
    , _service_announcement_h(service_announcement)
{
 ...
 // cfg.lookupValue("mw.http_server.api_path", _api_path);
  ...
  _listener->support(methods::OPTIONS, std::bind(&RestHandler::options, this, std::placeholders::_1));
  _listener->open().wait();
}

MBMS_RT::RestHandler::~RestHandler() = default;

void MBMS_RT::RestHandler::options(http_request message) {
    web::http::http_response response(status_codes::NoContent);
    response.headers().add(U("Access-Control-Allow-Origin"), "*");
    response.headers().add(U("Access-Control-Allow-Methods"), "GET, POST, OPTIONS");
    response.headers().add(U("Access-Control-Allow-Headers"), "range");
    ...
    if (paths[0] == _api_path) {
      ...
      if (it != _cache.item_map().cend()) {
        if (it->second->buffer() != nullptr) {
          ...
          response.headers().add(U("Access-Control-Allow-Origin"), "*");
          response.headers().add(U("Access-Control-Expose-Headers"), "*");
