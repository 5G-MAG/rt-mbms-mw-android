...
auto MBMS_RT::Service::stop() -> void
{
...
void MBMS_RT::Service::handle_received_data(const signed char* buffer, size_t size) {
  for (const auto& stream : _content_streams) {
    stream.second->handle_received_data(buffer, size);
  }
};
